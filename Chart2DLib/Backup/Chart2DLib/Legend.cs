...
Category("Appearance")]
        ...
        public void AddLegend(Graphics g,
        DataCollection dc, ChartStyle cs)
        {
            ...
            switch (LegendPosition)
            {
                case LegendPositionEnum.East:
                    xc = cs.PlotArea.X + cs.PlotArea.Width -
                    offSet - hWidth;
                ...
                case LegendPositionEnum.South:
                    xc = cs.PlotArea.X + cs.PlotArea.Width / 2;
                    yc = cs.PlotArea.Y + cs.PlotArea.Height -
                    offSet - hHeight;
                    break;
    ...
    DataCollection dc, ChartStyle cs)
        {
            ...
            if (isLegendVisible)
            {
                ...
                foreach (DataSeries ds in dc.DataSeriesList)
                {
                    ...
                    float xSymbol = legendRectangle.X +
                    spacing + hlineLength;
                    float xText = legendRectangle.X +
                    2 * spacing + lineLength;
                    float yText = legendRectangle.Y + n * spacing +
                    (2 * n - 1) * htextHeight;
                    aPen = new Pen(ds.LineStyle.LineColor,
                    ds.LineStyle.Thickness);
                    aPen.DashStyle = ds.LineStyle.Pattern;
                    PointF ptStart = new PointF(legendRectangle.X +
                    spacing, yText);
                    PointF ptEnd = new PointF(legendRectangle.X +
